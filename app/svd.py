...
class SVDReader:
    ...
    def get_packed_list(self):
        ...
        for vendor in vendors:
            ...
            packed += [{"vendor": vendor, "filenames": sorted(filenames)}]
        return sorted(packed, key=lambda k: k["vendor"])

    def parse_path(self, path):
        self.__fill_device(
            [periph for periph in SVDParser.for_xml_file(path).get_device().peripherals]
        )
            ...
            self.device += [
                {
                    ...
                    "regs": periph.derived_from,
                }
            ]  # regs value will be replaced with regs list
            if self.device[-1]["regs"] is not None:
                self.device[-1]["regs"] = next(
                    periph
                    for periph in self.device
                    if periph["name"] == self.device[-1]["regs"]
                )["regs"].copy()
            else:
                self.device[-1]["regs"] = []
